 ...
 * @returns {boolean}
 */
export function isTransactionNFTCreation(tx: Transaction): boolean {
  /*
 ...
 */
function generateNFTTokenMetadataJSON(nftUid: string): Record<string, { id: string, nft: boolean }> {
  const nftMetadata = {};
  nftMetadata[nftUid] = {
    id: nftUid,
    nft: true,
