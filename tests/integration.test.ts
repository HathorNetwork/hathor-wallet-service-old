...
block.timestamp = now;
block.height = 1;
block.outputs = [createOutput(0, blockReward, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62')];
...
block2.outputs = [createOutput(0, blockReward, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62')];
...
tx.tx_id = txId3;
tx.timestamp += 20;
tx.inputs = [createInput(blockReward, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', txId1, 0)];
tx.outputs = [
  createOutput(0, blockReward - 5000, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc'),
  createOutput(1, 5000, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q'),
];
...
tx2.tx_id = txId4;
tx2.timestamp += 20;
tx2.inputs = [createInput(5000, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', txId2, 1)];
tx2.outputs = [
  createOutput(0, 1000, 'HLfGaQoxssGbZ4h9wbLyiCafdE8kPm6Fo4', '00', timelock),   // belongs to this wallet
...
const checkAfterReceivingFirstBlock = async (walletStarted = false) => {
  ...
  await expect(checkUtxoTable(mysql, 1, txId1, 0, htrToken, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', blockReward, 0, null, block.height + blockRewardLock, true)).resolves.toBe(true);
  await expect(checkAddressBalanceTable(mysql, 1, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', htrToken, 0, blockReward, null, 1)).resolves.toBe(true);
  await expect(checkAddressTxHistoryTable(mysql, 1, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', txId1, htrToken, blockReward, block.timestamp)).resolves.toBe(true);
  if (walletStarted) {
    ...
    await expect(checkAddressTable(mysql, maxGap + 1, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', 0, walletId, 1)).resolves.toBe(true);
    ...
    await expect(checkAddressTable(mysql, 1, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', null, null, 1)).resolves.toBe(true);
  }
};
...
const checkAfterReceivingSecondBlock = async (walletStarted = false) => {
  const blockRewardLock = parseInt(process.env.BLOCK_REWARD_LOCK, 10);
  await expect(checkUtxoTable(mysql, 2, txId2, 0, htrToken, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', blockReward, 0, null, block2.height + blockRewardLock, true)).resolves.toBe(true);
  // first block utxo is unlocked
  await expect(checkUtxoTable(mysql, 2, txId1, 0, htrToken, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', blockReward, 0, null, block.height + blockRewardLock, false)).resolves.toBe(true);
  await expect(checkAddressBalanceTable(mysql, 1, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', htrToken, blockReward, blockReward, null, 2)).resolves.toBe(true);
  await expect(checkAddressTxHistoryTable(mysql, 2, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', txId1, htrToken, blockReward, block.timestamp)).resolves.toBe(true);
  await expect(checkAddressTxHistoryTable(mysql, 2, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', txId2, htrToken, blockReward, block2.timestamp)).resolves.toBe(true);
  if (walletStarted) {
    await expect(checkWalletTable(mysql, 1, walletId, WalletStatus.READY)).resolves.toBe(true);
    ...
    await expect(checkAddressTable(mysql, maxGap + 1, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', 0, walletId, 2)).resolves.toBe(true);
    // addresses other than the used on must have been added to address table
    for (let i = 1; i < maxGap + 1; i++) {
      await expect(checkAddressTable(mysql, maxGap + 1, ADDRESSES[i], i, walletId, 0)).resolves.toBe(true);
    }
    ...
    await expect(checkWalletTxHistoryTable(mysql, 0)).resolves.toBe(true);
    await expect(checkWalletBalanceTable(mysql, 0)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, 1, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', null, null, 2)).resolves.toBe(true);
  }
};
...
const checkAfterReceivingTx1 = async (walletStarted = false) => {
  await expect(checkUtxoTable(mysql, 3, txId3, 0, htrToken, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', blockReward - 5000, 0, null, null, false)).resolves.toBe(true);
  await expect(checkUtxoTable(mysql, 3, txId3, 1, htrToken, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', 5000, 0, null, null, false)).resolves.toBe(true);
  await expect(checkAddressBalanceTable(mysql, 3, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', htrToken, 0, blockReward, null, 3)).resolves.toBe(true);
  await expect(checkAddressBalanceTable(mysql, 3, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', htrToken, blockReward - 5000, 0, null, 1)).resolves.toBe(true);
  await expect(checkAddressBalanceTable(mysql, 3, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', htrToken, 5000, 0, null, 1)).resolves.toBe(true);
  // 3 new entries must have been address to address_tx_history
  await expect(checkAddressTxHistoryTable(mysql, 5, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', txId3, htrToken, (-1) * blockReward, tx.timestamp)).resolves.toBe(true);
  await expect(checkAddressTxHistoryTable(mysql, 5, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', txId3, htrToken, blockReward - 5000, tx.timestamp)).resolves.toBe(true);
  await expect(checkAddressTxHistoryTable(mysql, 5, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', txId3, htrToken, 5000, tx.timestamp)).resolves.toBe(true);
  if (walletStarted) {
    await expect(checkWalletTable(mysql, 1, walletId, WalletStatus.READY)).resolves.toBe(true);
    ...
    await expect(checkAddressTable(mysql, maxGap + 3, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', 0, walletId, 3)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, maxGap + 3, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', 1, walletId, 1)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, maxGap + 3, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', 2, walletId, 1)).resolves.toBe(true);
  } else {
    await expect(checkWalletTable(mysql, 0)).resolves.toBe(true);
    await expect(checkWalletTxHistoryTable(mysql, 0)).resolves.toBe(true);
    await expect(checkWalletBalanceTable(mysql, 0)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, 3, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', null, null, 3)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, 3, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', null, null, 1)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, 3, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', null, null, 1)).resolves.toBe(true);
  }
};
...
const checkAfterReceivingTx2 = async (walletStarted = false) => {
  await expect(checkUtxoTable(mysql, 5, txId3, 0, htrToken, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', blockReward - 5000, 0, null, null, false)).resolves.toBe(true);
  await expect(checkUtxoTable(mysql, 5, txId4, 0, htrToken, 'HLfGaQoxssGbZ4h9wbLyiCafdE8kPm6Fo4', 1000, 0, timelock, null, true)).resolves.toBe(true);
  ...
  await expect(checkAddressBalanceTable(mysql, 5, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', htrToken, 0, blockReward, null, 3)).resolves.toBe(true);
  await expect(checkAddressBalanceTable(mysql, 5, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', htrToken, blockReward - 5000, 0, null, 1)).resolves.toBe(true);
  await expect(checkAddressBalanceTable(mysql, 5, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', htrToken, 0, 0, null, 2)).resolves.toBe(true);
  await expect(checkAddressBalanceTable(mysql, 5, 'HLfGaQoxssGbZ4h9wbLyiCafdE8kPm6Fo4', htrToken, 0, 1000, timelock, 1)).resolves.toBe(true);   // locked
  ...
  // 3 new entries must have been address to address_tx_history
  await expect(checkAddressTxHistoryTable(mysql, 8, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', txId4, htrToken, -5000, tx2.timestamp)).resolves.toBe(true);
  await expect(checkAddressTxHistoryTable(mysql, 8, 'HLfGaQoxssGbZ4h9wbLyiCafdE8kPm6Fo4', txId4, htrToken, 1000, tx2.timestamp)).resolves.toBe(true);
  ...
  if (walletStarted) {
    ...
    await expect(checkAddressTable(mysql, maxGap + 7 + 1, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', 0, walletId, 3)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, maxGap + 7 + 1, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', 1, walletId, 1)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, maxGap + 7 + 1, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', 2, walletId, 2)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, maxGap + 7 + 1, 'HLfGaQoxssGbZ4h9wbLyiCafdE8kPm6Fo4', 6, walletId, 1)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, maxGap + 7 + 1, 'HCuWC2qgNP47BtWtsTM48PokKitVdR6pch', null, null, 1)).resolves.toBe(true);
  } else {
    await expect(checkWalletTable(mysql, 0)).resolves.toBe(true);
    await expect(checkWalletTxHistoryTable(mysql, 0)).resolves.toBe(true);
    await expect(checkWalletBalanceTable(mysql, 0)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, 5, 'HBCQgVR8Xsyv1BLDjf9NJPK1Hwg4rKUh62', null, null, 3)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, 5, 'HPDWdurEygcubNMUUnTDUAzngrSXFaqGQc', null, null, 1)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, 5, 'HEYCNNZZYrimD97AtoRcgcNFzyxtkgtt9Q', null, null, 2)).resolves.toBe(true);
    await expect(checkAddressTable(mysql, 5, 'HLfGaQoxssGbZ4h9wbLyiCafdE8kPm6Fo4', null, null, 1)).resolves.toBe(true);
